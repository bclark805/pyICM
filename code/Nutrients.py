...
def DTNO3(NH4, NO3, T, DO2, NO3A, Q, V, FR, RivNO3):
...
def DTNH4(NH4, NO3, T, DO2, MNLDON, NH4A, Q, V, FR, RivNH4):
...
def Nitrification(NH4, T, DO2):
    ...
    NH4lim = NH4/(dv.KHNNT+NH4)
    # nitrifcation in units of g N m^-3 d^-1
    NT = O2lim*NH4lim*FTNT*dv.NTM*NH4

    return NT


def Npref(NH4, NO3, KHN):
    ...
    LeftSide = NO3/((KHN+NH4)*(KHN+NO3))
    # Right side of Function (Clark et al. 2020)
    RightSide = KHN/(DIN*(KHN+NO3))
    # Nitrogen preference equation
    PN = NH4*(LeftSide+RightSide)
